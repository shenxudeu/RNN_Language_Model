...
class RNN:
    ...
    def bptt(self, x, y):
        ...
        ###### BACKWARD PASS
        dL = 1.
        # backward gate L = cross_entropy(Y, softmax(o))
        # We computed dO for every time-steps
        do = p * dL
        do[np.arange(len(y)),y] -= 1.
        
        # For each output backwards...
        for t in np.arange(T)[::-1]:
            # backward gate o_t = V. S_t
            dLdV += np.outer(do[t],s[t].T)
            ds_t  = self.V.T.dot(do[t])
            # backward gate s_t = tanh(spx_t)
            dspx_t = ds_t * (1 - (s[t]**2))
            
            # Backpropagation through time (for at most self.bptt_truncate steps)
            for bptt_step in np.arange(max(0, t-self.bptt_truncate),t+1)[::-1]:
                # backward gate spx_t = W.S_{t-1} + U.X_t
                dLdW += np.outer(dspx_t, s[bptt_step-1])
                dLdU[:,x[bptt_step]] += dspx_t
                ds_t_minus_1 = self.W.T.dot(dspx_t)
                # backward gate s_{t-1} = tanh(spx_{t-1})
                dspx_t = ds_t_minus_1 * (1 - s[bptt_step-1] **2)
